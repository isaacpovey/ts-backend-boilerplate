...
exports.accessLogMiddleware = async (ctx, next) => {
    ...
    try {
        ...
        console.info(`${start.toISOString()} ${xForwardedFor} ${remoteIp} ${requestId} ${userId} - ${req.method} ${reqHost}${req.url} HTTP/${httpVersion} ${res.status} ${resLength} ${diff} "${referrer}" "${userAgent}"`);
    }
};
